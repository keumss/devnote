...
학습 콘텐츠는 `content/<section>/<document>.mdx`에 있습니다. 같은 폴더에 제목 frontmatter를 가진 문서를 추가하면 파일명의 숫자를 인식하는 이름순으로 정렬되고, 내비게이션과 검색 인덱스에 자동 반영됩니다. 섹션 표시 이름은 각 폴더의 `meta.json`, 섹션 순서는 `content/meta.json`에서 관리합니다.
